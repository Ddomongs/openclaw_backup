...
const reason = String(process.argv.slice(3).join(' ') || '').trim() || 'unknown';

if (!approvalId) {
  console.error('usage: node ./scripts/navertalk-delivery-queue-fail.mjs <approvalId> <reason...>');
  process.exit(1);
}
...
if (approval) {
  ...
  approval.queue.state = 'failed';
  approval.queue.failedAt = now;
  approval.queue.reason = reason;
