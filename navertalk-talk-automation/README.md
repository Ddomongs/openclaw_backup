...
- 팝업에서 주문/구매이력 추출
- 간단한 CS 초안 자동 생성
- 팝업에서 Discord 승인 카드 초안 생성 (`/api/approvals` 연동)
...
7. 필요 시 `승인 카드 생성` 버튼으로 Discord 승인 대기 메시지 초안 생성

## 팝업 승인 카드 생성 조건
- 리스트 화면에서 먼저 `매핑 저장`이 되어 있어야 팝업에서 webhook 카드 userId를 찾을 수 있습니다.
- 팝업에서는 현재 연결된 `userId`를 표시합니다.
- `승인 카드 생성` 버튼을 누르면 `navertalk-monitor /api/approvals`에 저장되고, Discord 보고용 메시지 문자열이 생성됩니다.
