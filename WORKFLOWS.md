...
7. 송장번호 입력 후 **바로 `Tab`** 을 눌러 하단 자동 파싱/고객 답변 초안 생성을 유도한다.
8. 하단에 자동 생성된 `고객 답변 초안`과 `통관 예상 / 배송 예상 / 조회 링크`를 확인한다.
9. 배송문의 답변은 **반드시 이 하단 자동 생성 결과를 기준으로** 승인 카드/초안을 작성한다.
...
6. 적재 직후 `delivery queue worker` 를 `cron run`으로 즉시 1회 실행한다.
   - 즉시 실행 실패 시에도 큐는 유지하고, 느린 백업 재시도가 후속 처리한다.
7. 대표는 이전 건 완료 보고를 기다리지 않고 다음 승인 카드를 계속 처리한다.
8. 로컬 브라우저 자동화는 큐에서 한 건씩 꺼내 톡톡 파트너센터를 열어 해당 고객 상담을 찾고 초안을 입력/전송한다.
9. 전송이 끝나면 부분 캡처와 함께 완료 보고를 올리고 다음 큐 건으로 이동한다.
